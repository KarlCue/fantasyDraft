...
def populate_stats():
    try:
        ...
        pick = requests.get(f"{app_config['eventstore']['url']}/game/draft/readings", params={"start_timestamp" : stats['last_updated'] , "end_timestamp" : current_datetime})
        trade = requests.get(f"{app_config['eventstore']['url']}/game/trades/readings", params={"start_timestamp" : stats['last_updated'] , "end_timestamp" : current_datetime})
        data = pick.json()
