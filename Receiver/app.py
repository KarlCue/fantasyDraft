...
def add_pick(body):
    ...
    logger.info(f'Received picks request with trace id of {body["trace_id"]}')
    ...
    logger.info(f'Returned picks response (id: {body["trace_id"]}) with status code 201')
...
def add_trade(body):
    trace_id = str(uuid.uuid4())
    body["trace_id"] = trace_id
    logger.info(f'Received trades request with trace id of {body["trace_id"]}')
    ...
    logger.info(f'Returned trades response (id: {body["trace_id"]}) with status code 201')
    return NoContent, 201
